...
| path | what |
| --- | --- |
| `server.mjs` | zero-dep proxy: strips `embedding` vectors from entry payloads (~85% of bytes), serves `dist/` at `/`, legacy app at `/legacy/`; for the memory tool it also keeps a rotating LTM restore point before each run's first write (`.backups/`), stores review decisions (`.state/`), and forwards `MARINARA_ADMIN_SECRET` for engines off loopback |
