...
export function EntryDrawer(props: {
  ...
  const advChanged = ADVANCED_FIELDS.filter(
    ([f, d]) => e[f] !== undefined && JSON.stringify(e[f]) !== JSON.stringify(d),
  );
      ...
      <div className={`sub ${isOpen ? "is-open" : ""} ${flag ? "has-error" : ""}`}>
        <button className="sub-head" aria-expanded={isOpen} onClick={() => toggle(id)}>
          <span className="t-label t-label-s">
            {label}
            {flag && (
              <span className="err-dot" aria-label={t("lorebooks.entry.hasError")}>
                ●
              </span>
            )}
          </span>
  ...
  return (
    <div className="drawer" data-s={status}>
      {sub(
        "keys",
        t("lorebooks.entry.subKeys"),
        e.keys.length ? (
          <>
            <b>{e.keys.length}</b> · {e.keys.slice(0, 3).join(", ")}
            {e.keys.length > 3 ? "…" : ""}
          </>
        ) : (
          ...
          <div className="kchips">
            {e.keys.map((k, i) => (
              <span key={`${k}:${i}`} className={`kchip ${props.evHits.includes(k) ? "is-hit" : ""}`}>
                ...
                <button
                  className="x"
                  aria-label={t("lorebooks.entry.removeKey", { value1: k })}
                  onClick={() =>
                    set(
                      "keys",
                      e.keys.filter((_, j) => j !== i),
                    )
                  }
                >
      ...
      {sub(
        ...
        <>
          <b>{(e.description ?? "").length}</b> {t("ui.editor.charUnit")} · <b>{tokensOf(e.description)}</b>{" "}
          {t("lorebooks.unitTokens")}
        </>,
        () => (
          <>
            ...
            <textarea
              className={`ta ${isDirty("description") ? "is-dirty" : ""}`}
              rows={4}
              value={e.description}
      ...
      {sub(
        ...
        <>
          <b>{(e.content ?? "").length}</b> {t("ui.editor.charUnit")} · <b>{entryTokens(e)}</b>{" "}
          {t("lorebooks.unitTokens")}
        </>,
        () => (
          <>
            ...
            <textarea
              className={`ta is-mono ${isDirty("content") ? "is-dirty" : ""}`}
              rows={7}
              value={e.content}
      ...
      {sub(
        "trigger",
        t("lorebooks.entry.subTrigger"),
        <>
          <span className="st">{STATUS_LABEL[status]}</span> · {POS_COMPACT[e.position] ?? ""} ·{" "}
          {t("lorebooks.field.order")} <b>{e.order}</b>
        </>,
        () => (
          <>
            <div className="seg4">
                  ...
                  onClick={() =>
                    merge(
                      v === "disabled"
                        ? { enabled: false }
                        : v === "constant"
                          ? { enabled: true, constant: true, selective: false }
                          : v === "selective"
            ...
            <div className="movebar">
              <button
                aria-label={t("lorebooks.entry.lowerOrder")}
                onClick={() => set("order", Math.max(0, e.order - 10))}
              >
                −
              </button>
              <span className="slot">
                <input
                  className="ordin t-num"
                  type="number"
                  value={e.order}
                  aria-label={t("lorebooks.field.order")}
                  onInput={(ev) => set("order", Number(ev.currentTarget.value))}
                />
                <span className="c">
                  {POS_FULL[e.position] ?? ""}
                  {e.position === 2 ? ` ${e.depth}` : ""}
                  {e.position === 7 ? ` ${e.outletName || "—"}` : ""}
                </span>
              </span>
              <button aria-label={t("lorebooks.entry.raiseOrder")} onClick={() => set("order", e.order + 10)}>
                ＋
              </button>
            </div>
          </>
          ...
          <>
            ...
            <div className="advrow">
              ...
              <span className="av t-data">
                {t("lorebooks.entry.updatedAtUtc", {
      ...
      {sub(
        "name",
        t("lorebooks.entry.nameLabel"),
        <span className="t-data">{e.name || t("lorebooks.entry.untitled")}</span>,
        () => (
          <>
            <input
              className={`tin ${isDirty("name") ? "is-dirty" : ""}`}
              value={e.name}
              placeholder={t("lorebooks.entry.untitled")}
