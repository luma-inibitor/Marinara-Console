...
export function isPatternSafe(source, options = {}) {
    ...
    if (source.length > maxLength)
        return false;
    if (hasPolynomialBacktrackingRisk(source, maxRepetition))
        return false;
    ...
    const recordAtomHeight = (h) => {
        if (groupDepth > 0) {
            ...
            if (h > groupInnerHeight[idx])
                groupInnerHeight[idx] = h;
        }
    ...
    while (i < source.length) {
        ...
        if (c === "\\") {
            ...
            if (consumed === INVALID_QUANTIFIER)
                return false;
        ...
        if (c === ")") {
            ...
            if (quantified && hasUnsafeQuantifiedAlternation(source.slice(bodyStart, i)))
                return false;
            const groupHeight = innerHeight + (quantified ? 1 : 0);
            if (groupHeight > maxStarHeight)
                return false;
    ...
    if (topLevelHeight > maxStarHeight)
        return false;
    return true;
}
...
function consumeQuantifier(source, i, maxRepetition) {
    ...
    if (c === "{") {
        ...
        if (!Number.isFinite(lo) || lo > maxRepetition)
            return INVALID_QUANTIFIER;
        if (Number.isFinite(hi) && hi > maxRepetition)
            return INVALID_QUANTIFIER;
        if (!Number.isFinite(hi) && Number.isFinite(maxRepetition))
            return INVALID_QUANTIFIER;
        let next = close + 1;
        if (source[next] === "+")
            return INVALID_QUANTIFIER;
...
function hasUnsafeQuantifiedAlternation(body) {
    ...
    for (let i = 0; i < tokenized.length; i += 1) {
        for (let j = i + 1; j < tokenized.length; j += 1) {
            const a = tokenized[i];
            const b = tokenized[j];
            if (isTokenPrefix(a, b) || isTokenPrefix(b, a))
                return true;
        }
    }
    return false;
    ...
    for (let i = 0; i < source.length;) {
        ...
        if (c === "\\") {
            ...
            if (broadUnbounded) {
                ...
                if (adjacentBroadUnboundedCount >= 2 || broadUnboundedCount >= 3)
                    return true;
            }
        ...
        if (consumed === INVALID_QUANTIFIER)
            return true;
        if (isRequiredLiteralAtom(consumed))
            consumeRequiredLiteralBoundary(c);
        i = consumed?.end ?? atomEnd;
    }
    return false;
}
