...
- **No sheet traps focus**, though every sheet reads as if it does. `src/shell/background.ts` now freezes and inerts the page behind an open surface, so Tab cannot leave it. Focus still does not wrap at the end of a surface or land inside it on open.
