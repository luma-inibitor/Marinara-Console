...
- **Every engine reply the console reads is now parsed, and three write paths are parsed unobserved.** Valibot schemas guard the whole memory surface through `src/shell/wire.ts` and `src/tools/memory/api/schema.ts`, and `api/types.ts` holds no hand-written interface: every type there is inferred from the schema that checks it. The reads — `GET /notes`, `GET /notes/:id`, `GET /drafts/review`, `GET /status`, `POST /import/preview` and `POST /drafts/:id/preflight` (both POSTs move nothing, so they parse as reads) — plus the host's `GET /chats` and `GET /characters` and the console's own ledger under `/console/state/ltm-review` were each run against the dev engine's live corpus. The writes parse whole or throw. `lorebooks/api/schema.ts` and `presets/api/schema.ts` parse too, each beside the endpoint functions it guards: a lorebook row is JSON so its flags are `v.boolean()`, a preset row is TEXT columns so `wireBool` takes exactly `true`/`false`/`"true"`/`"false"` and `jsonText` decodes a JSON column against the shape the tool needs. What is still unparsed is only what nothing reads: `POST /rebuild`, `backup/export` (a download URL, so no payload passes through the console at all), and the tool writes `bulkPatch`, `patchPreset` and `setDefaultPreset`.
- **Neither tool owns its entity state.** The two `data.ts` files are split — both tools now have `api/`, `model/` and `store/`, and `layercheck` classifies every file in them — but the split moved only the layers, not the ownership: `BookAudit.tsx` and `PresetsTool.tsx` still hold the loaded entries and the loaded preset in `useState` and refetch by hand after a write, so `store/books.ts` and `store/presets.ts` are a boundary presentation may call rather than an owner in the sense of ARCHITECTURE §1. Giving them the memory tool's shape is a behavior change (invalidation moves), so it was deliberately left out of the move.
- **`POST /import/source-notes`, `POST /drafts/:id/accept` and `POST /drafts/:id/skip` have schemas nobody has seen a live reply match.** All three mutate the vault irreversibly, so none was run against the dev engine. Their schemas are read off the engine package's own zod response schemas (long-term-memory 1.2.9), which is what the engine parses its own reply through before sending — stronger evidence than a recorded sample, but not the same as one. Each requires only the fields the console reads, so a mismatch throws after the change has landed rather than corrupting anything, and each wants one live write to settle.
- **The import report reads its tally off the wrong copy of it.** `ImportReport` in `Sources.tsx` reads `item.draft?.accounting`, and the engine marks that field optional on a draft while the imported entry beside it carries the same tally required. A draft that omits it makes the report say kept 0 / rejected 0 for a source that imported fine. The fix is `item.draft?.accounting ?? item.accounting`, and it is not in the validation PR because the import path cannot be exercised here to confirm the entry-level field arrives populated.
...
- **Most of what the engine sends is still an unchecked cast.** Valibot schemas now guard the read paths the vault and the review queue depend on — `GET /notes`, `GET /notes/:id`, `GET /drafts/review` — and the three note write paths — `PATCH /notes/:id`, `DELETE /notes/:id`, `POST /notes/:id/extract` — through `src/shell/wire.ts` and `src/tools/memory/api/schema.ts`. Writes parse whole or throw (`parseWrite`); the `putNote` guard that patched the `{note, rebuild}` defect is gone, its job now done by `NoteWriteSchema` at the boundary. `lorebooks/data.ts` and `presets/data.ts` — where the `"false"` bug actually happened — now parse too, in schemas beside their endpoint functions: a lorebook row is JSON so its flags are `v.boolean()`, a preset row is TEXT columns so `wireBool` takes exactly `true`/`false`/`"true"`/`"false"` and `jsonText` decodes a JSON column against the shape the tool needs. Everything else still travels on `as T`: `drafts.ts` preflight/accept/skip, `import.ts`, `status.ts`, `backup.ts`, `chats.ts`, `characters.ts`, `ledger.ts`, and the tool writes whose replies nothing reads (`bulkPatch`, `patchPreset`, `setDefaultPreset`). Extend the same pattern outward; the hand-written interfaces still in `api/types.ts` mark what is left.
- **The two tool `data.ts` files are still UNCLASSIFIED to `layercheck`**, because they mix wire types, endpoint functions and domain transforms in one module — the tangle `memory/data.ts` was split out of. Their schemas sit beside their fetchers rather than under an `api/` they do not have, so nothing about them is layer-checked. The shape is a split into `api/` + `model/` per ARCHITECTURE §2, and it is a layering change rather than a validation one.
- **Most of what the engine sends is still an unchecked cast.** Valibot schemas now guard the read paths the vault and the review queue depend on — `GET /notes`, `GET /notes/:id`, `GET /drafts/review` — and the three note write paths — `PATCH /notes/:id`, `DELETE /notes/:id`, `POST /notes/:id/extract` — through `src/shell/wire.ts` and `src/tools/memory/api/schema.ts`. Writes parse whole or throw (`parseWrite`); the `putNote` guard that patched the `{note, rebuild}` defect is gone, its job now done by `NoteWriteSchema` at the boundary. `lorebooks/api/schema.ts` and `presets/api/schema.ts` — where the `"false"` bug actually happened — now parse too: a lorebook row is JSON so its flags are `v.boolean()`, a preset row is TEXT columns so `wireBool` takes exactly `true`/`false`/`"true"`/`"false"` and `jsonText` decodes a JSON column against the shape the tool needs. Everything else still travels on `as T`: `drafts.ts` preflight/accept/skip, `import.ts`, `status.ts`, `backup.ts`, `chats.ts`, `characters.ts`, `ledger.ts`, and the tool writes whose replies nothing reads (`bulkPatch`, `patchPreset`, `setDefaultPreset`). Extend the same pattern outward; the hand-written interfaces still in `api/types.ts` mark what is left.
- **Neither tool owns its entity state.** The two `data.ts` files are split — both tools now have `api/`, `model/` and `store/`, and `layercheck` classifies every file in them — but the split moved only the layers, not the ownership: `BookAudit.tsx` and `PresetsTool.tsx` still hold the loaded entries and the loaded preset in `useState` and refetch by hand after a write, so `store/books.ts` and `store/presets.ts` are a boundary presentation may call rather than an owner in the sense of ARCHITECTURE §1. Giving them the memory tool's shape is a behavior change (invalidation moves), so it was deliberately left out of the move.
- **No PATCH or POST reply on either tool route family has been observed.** The dev engine holds a writer lease, so the write schemas are read off what the console already assumed — `createSection` and `duplicatePreset` normalized their replies as bare rows — plus `v.nullish` for a 204. If the engine wraps a reply in an envelope, saving throws a wire mismatch instead of corrupting the row, which is the safe direction to be wrong in, but it wants one live write to settle.
...
- **`noUncheckedIndexedAccess`.** Measured at roughly 60 diagnostics, half a day. Several look like live bugs, including a percentile lookup in `lorebooks/model/audit.ts` and a diff-op scan in `ClaimDetail.tsx`. Deferred only until the review-queue redesign lands, to avoid colliding with it.
