...
- **Most of what the engine sends is still an unchecked cast.** Valibot schemas now guard the read paths the vault and the review queue depend on — `GET /notes`, `GET /notes/:id`, `GET /drafts/review` — through `src/shell/wire.ts` and `src/tools/memory/api/schema.ts`. Everything else still travels on `as T`: the note write paths (`PATCH`/`DELETE`/`extract`, whose `{note, rebuild}` envelope was fixed once by a hand-written guard in `putNote`), `drafts.ts` preflight/accept/skip, `import.ts`, `status.ts`, `backup.ts`, `chats.ts`, `characters.ts`, `ledger.ts`, and `lorebooks/data.ts` + `presets/data.ts` — which is where the `"false"` bug actually happened, and where a `v.boolean()` would have caught it. Extend the same two-mode pattern outward; the hand-written interfaces still in `api/types.ts` mark what is left.
- **`FullscreenText`, `Palette` and `CheatSheet` still render `aria-modal="true"` without a trap.** `Sheet`/`Modal` were fixed on `fix/sheet-focus-trap` by putting Radix's dialog inside `ui/Sheet.tsx`; these three are separate surfaces that each need their own pass, and `FullscreenText`'s microtask re-registration is delicate enough to be its own change. The scroll-lock half of the original finding did not reproduce: the console has no document-level scroll and the fixed full-viewport scrim absorbs wheel and touch, so nothing behind a sheet was scrollable to begin with.
