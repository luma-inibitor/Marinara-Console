...
- **The group menu does not take focus on open.** A sealed surface now places focus on its first control when it opens and wraps Tab at both ends (`src/shell/overlays.ts`). The group menu is anchored rather than sealed, so focus stays on its button until the reader tabs into the menu.
...
- **Thirteen sub-floor contrast findings on three screens**, recorded in `design/contrast-baseline.json` by `tests/e2e/contrast.spec.ts` per screen and viewport (52 entries) and failing only on growth. Six on the lorebook audit, six on the preset editor, one on the presets list. All of them are `--text-faint` or `--text-dim` on `--canvas` or `--surface-1`, between 1.69:1 and 3.88:1 against a 4.5:1 floor. `verify.mjs`'s own algorithm reports the same set, so this is not an axe artifact; the fixture corpus is what exposes them, because it carries a disabled preset section, `off` tags and a preset with no description, and the live corpus these screens were checked against carried none of the three. DESIGN.md §1 restricts `--text-faint` to decorative or >=12px non-essential text, which several of these are, so whether they are defects or exemptions is a decision for the token ladder.
