...
open. It points into `design.md` and `CHECKLIST.md` rather than repeating
them.
...
needless click is a tax. The interface is a dark instrument
panel, not a consumer app: data is the ornament, and computed numbers do the
visual work that decoration does elsewhere.

**The device that matters is a 486px-wide phone** (1080 physical, device pixel
ratio 2.22). Tuning only at 390 leaves the shipping width unchecked. Desktop
...
2. **Review.** Extraction produces *claims* — proposed mutations to the vault.
   You keep or drop each one. Apply is the only write. This is the work.
3. **Vault.** The memories that survived. Browsed, searched, edited, archived.

A fourth view, **Activity**, exists in design only. It's the prerequisite for
undo.

**The single most important design question** is the one the detail pane was
rebuilt around: *what does the vault look like after accepting this claim?* An
earlier version answered with all the data about the claim. The owner's verdict:

> I feel assaulted by information I have no idea what to do with.
...
| Surface | State |
|---|---|
| Navigation | Scope bar over the views. Four views, one word each, count badges |
...
- **Color carries three reserved axes.** Status (`ok`/`warn`/`danger`/`off`) mirrors
...
## 5. Settled requirements

Owner-confirmed. Don't re-litigate. `design.md` §3 has the full list.
...
- **Undo over confirm.** `confirm()` only for irreversible operations.
- Numbers come from real data and stay engine-faithful. Thresholds derive
  from the data, not from constants someone picked.
- Copy comes from the engine's own catalog (`ltm-en.json`) wherever the
  concept exists upstream. Anything new gets an `OURS` entry with a reason. The eslint copy rules enforce this at the call site.
...
- **Split components by role, not by shape.** `Chip` and `Tag` look identical
  but stay separate: one is pressable, the other isn't.
...
`dimensionChanges` carry ten relationship axes and appear on one. They carry
structure by design. Don't parse them out of the prose.
...
|---|---:|---|---|---|
| **character** | 3 | `core` always, plus optional `voice`, `backstory`, `habits`, `appearance` | 82 / 100 / 434 | `extracted_from`, `caused_by` |
...
- **Source is an order of magnitude longer than everything else** — a median of
  781 characters beside 83–174 elsewhere, with a maximum of 2,818. It needs
...
- **Index health** lost its home when the status line went away.
