...
## 1. Before designing — what the surface is for
...
- [ ] **Name the decision this surface serves, in one sentence.** If the answer
      ...
      *(The import result card led with a per-source ledger. What the reviewer
      actually needed: did it work, is there anything to fix, is it worth
      opening now. The ledger was audit material wearing a headline.)*
- [ ] **List every state before drawing one.** Empty, loading, running, partial
      ...
      *(Six specimens jumped from a confirm button to a finished result with no
      running state between them, for an action that spends minutes of model
      calls. Nobody drew the edit mode at the center of the curate path.)*
...
- [ ] **Check the domain before designing around it.** Query the engine, read
      ...
      *(A whole repair panel assumed a free "revalidate" action. The
...
- [ ] **Copy must be true about the product.** Check the mechanism before
      describing it.
      *(The copy on a failed extraction denied any import. Import saves the
      source note. Only extraction failed, and the catalog said so.)*
...
- [ ] **Say each fact once per screen.** Two sentences meaning the same thing is
      ...
      *(A help line and a bar note 6 inches apart both explained when an edit
      takes effect.)*
...
- [ ] **Add them up.** Every count on screen must reconcile with every other.
      *(A rail claiming 197 pending + 96 imported, with a total of 213. A
      header claiming 61 proposed memories over rows summing to 58.)*
- [ ] **Does any number appear twice?** Show one count where two can never
      differ.
      *(A "4 selected" chip beside an "Import and extract (4)" button. A
      "(16) ✨16" button. A footer repeating the three counts already in the
      rail.)*
...
- [ ] **One meaning per channel.** Accent means interactive. Color is never the
      ...
      *(An "edited" chip in accent, which the detail pane just established
...
- [ ] **Sibling controls share a height and don't wrap.**
      *(A save button beside a two-line discard-changes button.)*
...
- [ ] **Are the two layers separable at a glance?** The specimen is the object.
      ...
      *(Frame titles sat inside the wireframe boxes, so they read as part of the
      interface itself, and they duplicated the outside label.)*
- [ ] **Re-read your captions beside the render.** Do they describe what's
