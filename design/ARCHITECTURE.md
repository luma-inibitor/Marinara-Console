...
The rule is about what a store holds, not how it's reached. Stores stay exported, and nothing enforces this: `layercheck.mjs` reads the import graph, and `notes.set()` in a `.tsx` file is a call rather than an import. This one is on the reviewer until a check exists for it.
...
**Type-only imports are exempt, and point anywhere.** The wire types live in `api/` because they describe the wire — that's the endpoints layer's own vocabulary. The model has to name those shapes to transform them, which is an upward import by the rule above. It's allowed, because a type import erases at compile time: it creates no runtime edge and therefore no cycle. `layercheck.mjs` asks the TypeScript checker what each specifier binds. Only the ones that resolve to a value count, so the answer is the compiler's rather than a reading of the syntax. `verbatimModuleSyntax` (`tsconfig.json:12-15`) keeps the two in step by making the compiler enforce the marking.
