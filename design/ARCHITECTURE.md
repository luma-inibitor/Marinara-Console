...
- **The browser checks are the UI suite.** The Playwright specs under `tests/e2e/` and `domsnap.mjs` each assert on a real render, and `npm run test:e2e` runs the specs. `copycheck.mjs` moved off the browser and now reads source, so it runs with the static checks.
- **`npm test` is Vitest alone.** The browser suite needs the app running, so it sits behind `npm run test:e2e` for a person to start on purpose. `npm run check` runs the static checks, then Vitest, then the build.

## 5. Enforcement

The house habit is to encode a rule in a script rather than to trust a convention. `copycheck` guards copy, `deadcss` guards CSS, `typescale` guards the type scale. `scripts/checks.mjs` lists the static checks one name per line, and `npm run check:static` runs every one of them rather than stopping at the first failure.
...
Rule 2 has a gap of its own, and it's the one gap that weakens a rule §3 marks as enforced. Rule 2 counts `fetch` only as a free identifier, because `isMemberName` skips any `fetch` whose parent is a property access. That skip keeps `opts.refetch()` and `client.fetch` out of the results. `window.fetch("/api/x")` is a property access too, so a component that writes it that way produces no finding. The `no-restricted-globals` rule in `eslint.config.js` doesn't close the gap either, because it matches the bare global and a member access isn't one. A presentation file holding `await window.fetch("/api/x")` passes both checks and exits 0. The other two things rule 2 fails on have no such hole, because an import of `api/` or of the request binding has one spelling each.

**`scripts/deadcss.mjs`** reports CSS classes that no `.ts` or `.tsx` file appears to use. Dead rules bury the rules that still matter, but every hit is a judgement call, so the check fails only on **growth**. `design/deadcss-baseline.json` records today's set and a finding outside it exits 1. It runs in `npm run check`, and a clean pull request never sees it. Shrinking the baseline is an ordinary diff, and growing it takes `--adopt` plus a line in the pull request body.
