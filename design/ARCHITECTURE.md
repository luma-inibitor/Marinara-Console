...
- **The browser checks are the UI suite.** `verify.mjs`, `overlaycheck.mjs`, `domsnap.mjs` and the Playwright specs under `tests/e2e/` drive a real browser. `npm run check:browser` runs the first two, and `npm run test:e2e` runs the specs. One of those files checks no render at all. `tests/e2e/corpus.spec.ts` parses the fixture corpus with the app's own schemas and opens no page, and `playwright.config.ts` gives its project no browser. `copycheck.mjs` moved off the browser and now reads source, so it runs with the static checks.
- **`npm test` is Vitest alone.** The browser suite drives a real Chromium, which CI installs in a step of its own, so it runs as a separate job rather than inside `npm run check`. `verify.mjs` and `overlaycheck.mjs` need an app already served: the first takes the origin as `--url` and the second reads `MC_DEV_URL`. Playwright needs nothing running first, because the `webServer` block in `playwright.config.ts` builds the bundle and previews it. `npm run check` runs the static checks, then Vitest, then the build.
