...
Inside `src/`, four things tangled together. The memory tool now resolves all four, and the other two tools don't follow yet.
...
- `pressure.ts`, `tally.ts` and `apply.ts` sit at the end. Each of the three reads `decisions.ts` and `review.ts`. Two of them, `tally.ts` and `apply.ts`, also read `preflight.ts`. No module under `store/` reads any of the three.
...
| `test/` | none | factories and test setup, not tests |

`layercheck.mjs` reads that table first, then falls through four more steps, and the order of those steps matters.

1. A path containing `lib`, `copy`, `test` or `vendor` as a directory name is exempt, and rule 1 skips it entirely.
2. Anything under `src/ui/` counts as presentation.
3. Any remaining `.tsx` file counts as presentation wherever it sits. This is how the memory tool's loose screens at `src/tools/memory/*.tsx` get classified at all.
4. Any remaining file under `src/shell/` counts as transport.

Step 3 runs before step 4, so `src/shell/api.ts` is transport while `src/shell/App.tsx` is presentation, and `layercheck` prints `src/shell` as a mixed directory for that reason. A `.ts` file that reaches the end of the list falls through to **unclassified**, and §5 says what that costs.
...
src/
  ...
  styles/                   tokens plus one sheet per tool
  ...
  tools/
    memory/
      ...
      review/               sheets belonging to the review surface
        ...
        FilterSheet.tsx
        ViewSheet.tsx
      MemoryTool.tsx        the four screens and the tool's own components
      Vault.tsx             still sit loose at the tool root
      Review.tsx
      Sources.tsx
      ClaimDetail.tsx
      ScopeBar.tsx
...
- **No component calls `fetch`.** A screen gets data by calling a hook. *Enforced by `layercheck.mjs` rule 2, which fails on the global `fetch` outside `src/shell/`, on presentation importing an `api/` directory, and on presentation importing the request function from `src/shell/api.ts`.*
...
- **Assert catalog keys, not English.** A test that asserts a rendered sentence breaks on any rewording of the copy. Tests stub `t()` to return `key|param=value`, and `copycheck.mjs` guards the copy itself.
...
- **The browser checks are the UI suite.** `verify.mjs`, `overlaycheck.mjs` and `domsnap.mjs` each assert on a real render, and `npm run check:browser` runs the first two. `copycheck.mjs` moved off the browser and now reads source, so it runs with the static checks.
- **`npm test` is Vitest alone.** The browser suite needs the app running, so it sits behind `npm run check:browser` for a person to start on purpose. `npm run check` runs the static checks, then Vitest, then the build.

## 5. Enforcement

The house habit is to encode a rule in a script rather than to trust a convention. `copycheck` guards copy, `deadcss` guards CSS, `overlaycheck` guards dismissal. `scripts/checks.mjs` lists the static checks one name per line, and `npm run check:static` runs every one of them rather than stopping at the first failure.
...
**`scripts/deadexports.mjs`** finds symbols that a module exports but only uses where they're declared, re-exports included. The check judges `export { X } from "./x"` at the barrel, so a forwarding line nobody imports from is dead in the ordinary way. Over-exporting makes a module's real surface unreadable, but every hit is a judgement call, so the check fails only on **growth**. `design/deadexports-baseline.json` records today's set and a finding outside it exits 1. `deadcss` works the same way from `design/deadcss-baseline.json`. Both run in `npm run check`, and a clean pull request never sees either. Shrinking a baseline is an ordinary diff, and growing one takes `--adopt` plus a line in the pull request body.
...
1. **Where do screens live.** The memory tool's four screens still sit loose at the tool root beside its components, and no `screens/` directory exists. The question is whether a tool wants one once it has more than a handful, and whether a tool with fifteen wants grouping inside that.
