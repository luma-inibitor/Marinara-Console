...
- **The browser checks are the UI suite.** `domsnap.mjs` and the Playwright specs under `tests/e2e/` drive a real browser. `bun run domsnap` runs the first, and `bun run test:e2e` runs the specs. One of those files checks no render at all. `tests/e2e/corpus.spec.ts` parses the fixture corpus with the app's own schemas and opens no page, and `playwright.config.ts` gives its project no browser.
- **`bun run test` is Vitest alone.** The browser suite drives a real Chromium, which CI installs in a step of its own, so it runs as a separate job rather than inside `bun run check`. Playwright needs nothing running first, because the `webServer` block in `playwright.config.ts` builds the bundle and previews it. `domsnap.mjs` is the one that needs an app already served: it reads `MC_DEV_URL`. `bun run check` runs the static checks, then Vitest, then the build.

## 5. Enforcement

The house habit is to encode a rule in a script rather than to trust a convention. `copycatalog` guards the copy catalog, `deadcss` guards CSS, `layercheck` guards the dependency direction. `scripts/checks.mjs` lists the static checks one name per line, and `bun run check:static` runs every one of them rather than stopping at the first failure.

**Prettier owns whitespace in `.ts`, `.tsx`, `.mjs` and the hand-written config files.** `eslint.config.js` extends no stylistic preset by design, so nothing read whitespace in the app code before this. `format:check` fails the build. `bun run format` fixes every finding.

**The pre-commit hook in `.githooks/` formats staged files.** `scripts/prepare.mjs` points `core.hooksPath` at that directory, and `bun install` runs `prepare` afterward, so the hook arrives with a clone rather than with an instruction. `scripts/precommit.mjs` holds the logic, which keeps it linted and type-checked like the rest of `scripts/`. It formats a staged file and re-stages it. It refuses to format a file that also carries worktree edits, because `git add` on that file would commit the unstaged half. Staged Markdown goes through `prosecheck`. The hook stops on every finding, not on the error-level ones alone. The CI prose job stays advisory, so the hook is the one place a suggestion still gets read.
...
**knip** finds symbols that a module exports but nothing imports, dead re-exports included. It replaced a bespoke script in wave 3. An export that must stay without an importer carries `/** @public */`, which knip reads as a claim of intent. To list every one, run `grep -rn '@public' src scripts`. `deadcss` keeps the older arrangement: `design/deadcss-baseline.json` records today's set and a finding outside it exits 1. Both run in `bun run check`, and neither one reports anything on a clean pull request. Shrinking a baseline is an ordinary diff, and growing one takes `--adopt` plus a line in the pull request body.

The lint rules that forbid a hand-written class, a template literal in a class position and a raw `<button>` have a ratchet of their own. A file that still breaks one carries a single comment at its top, `/* eslint-disable <rule>, <rule> -- legacy */`, and `eslint.config.js` names no file. `ratchet` counts those comments per rule and holds each count to `design/ratchet-baseline.json`. A count that rises fails, and a count that falls fails until the baseline says the new number. Retiring a file means removing its rules from that comment and lowering the count, so two pull requests that each retire a file never edit the same line. `eslint.config.js` sets `reportUnusedDisableDirectives` to error, so a comment for a rule the file no longer breaks fails `lint`.
