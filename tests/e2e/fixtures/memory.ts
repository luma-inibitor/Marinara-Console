...
const memory = (source: string, over: Partial<Note> & Pick<Note, "id" | "type" | "title">): Note =>
  note({ links: [{ target: source, relation: "extracted_from" }], ...over });
...
const SOURCE_NOTES: Note[] = [
  note({
    ...
    sections: {
      body: {
        text: "Sea-fog sits in the harbour until midday. The tide boards are chalked at dawn and nobody trusts them after noon.",
      },
    },
  }),
  note({
    id: SRC_MARKET,
    type: "source",
    title: "Lorebook - Atlas of the Harbour: Fishmarket",
    provenance: { kind: "lorebook", sourceId: "lb-atlas:e-market" },
    sections: {
      body: {
        text: "Open before light, shut by ten. The stalls nearest the water pay the least rent and flood first.",
      },
    },
  }),
  note({
...
const MEMORIES: Note[] = [
  memory(SRC_HARBOUR, {
    ...
    keywords: ["harbour", "fog", "tide"],
    manualKeywords: ["fog line"],
    sections: {
      summary: {
        text: "Fog holds in the harbour until about midday and the lamps are useless past the fog line.",
        importance: "high",
      },
      detail: {
        text: "The bell, not the lamps, is what boats navigate by in the mornings. Crews treat the fog line as the edge of the harbour rather than the wall.",
      },
  ...
  memory(SRC_HARBOUR, {
    id: "mem-tide-boards",
    type: "world",
    title: "The tide boards are chalked at dawn and doubted by noon",
    keywords: ["tide", "boards"],
    sections: {
      summary: {
        text: "Chalk on slate at the harbour steps, rewritten each dawn and wrong often enough that nobody plans around them.",
      },
    },
  }),
  memory(SRC_HARBOUR, {
    id: "mem-bell",
    type: "world",
    title: "The tide bell rings twice for a return, three times for a wreck",
    sections: {
      summary: { text: "Three rings has not happened in eleven years, and everyone in the harbour knows the count." },
    },
  }),
  memory(SRC_MARKET, {
    id: "mem-market-hours",
    type: "world",
    title: "The fishmarket opens before light and shuts by ten",
    sections: {
      summary: {
        text: "The stalls nearest the water are cheapest and flood first, which is the whole economics of the place.",
      },
    },
  }),
  memory(SRC_CAST, {
    id: "mem-mira-post",
    type: "character",
    title: "Mira Vance has been harbourmaster for nine years",
  ...
  memory(SRC_CHAT, {
    id: "mem-tolley-refusal",
    type: "relationship",
    title: "Tolley will not cross after the bell",
    modes: ["conversation", "roleplay"],
    links: [
      { target: SRC_CHAT, relation: "extracted_from" },
      { target: "mem-mira-post", relation: "concerns" },
    ],
    sections: {
      summary: {
        text: "Not a negotiation and not superstition; he has said so twice and given no reason either time.",
      },
    },
  }),
  memory(SRC_CHAT, {
  ...
  memory(SRC_MARKET, {
    id: "mem-old-rent",
    type: "world",
    title: "Stall rents were raised last winter",
    status: "archived",
    sections: {
      summary: {
        text: "Superseded by the market-hours memory; kept archived to show the vault's filter has something to filter.",
      },
    },
  }),
];
...
export const REVIEW: ReviewResponse = {
  ...
  sources: [
          ...
          rows: [
            row(
              ...
              write(
                "m-fog-detail",
                "detail",
                "The bell is rung from the west wall, which is why it sounds nearer than it is.",
              ),
            ),
            row(
              "draft-harbour",
              write("m-fog-caution", "caution", "Do not have a character mention the fog lifting early; it never has."),
            ),
            row(
              "draft-harbour",
              write("m-fog-risk", "detail", "Lamp oil is rationed in winter and the west wall is lit last.", {
                risk: "medium",
                confidence: 0.61,
              }),
            ),
          ...
          rows: [
            row(
              ...
              write(
                "m-boards-1",
                "summary",
                "Two boards, not one: the steps and the ferry slip, and they disagree about twice a month.",
              ),
            ),
            row(
              "draft-harbour",
              write("m-boards-2", "continuity", "Mira signs both. The signature is the only reason anyone reads them."),
            ),
          ],
        },
      ],
    },
    {
      ...
      drafts: [
        {
          ...
          candidateRejections: [
            {
              reason: "duplicate",
              message: "Already stored as mem-mira-post.",
              snippet: "Harbourmaster for nine years.",
            },
          ],
        },
      ],
      targets: [
        {
          noteId: "mem-mira-post",
          title: "Mira Vance has been harbourmaster for nine years",
          noteType: "character",
          rows: [
            row(
              "draft-mira",
              write("m-mira-1", "summary", "Nine years, and she took the post after the wreck rather than before it."),
            ),
            row(
              "draft-mira",
              mutation({
                id: "m-mira-kw",
                kind: "set_keywords",
                summary: "Add keywords: boards, signature",
                keywords: ["boards", "signature"],
              }),
            ),
          ],
        },
        {
          noteId: "mem-new-merrow",
          title: "Merrow keeps the far-shore light",
          noteType: "character",
          rows: [
            row(
              "draft-mira",
              mutation({
                ...
                note: note({
                  ...
                  sections: {
                    summary: {
                      text: "Named twice on Tuesday and never seen. Keeps the light on the far shore and does not cross either.",
                    },
                  },
                }),
              }),
    ...
    },
    {
      ...
      drafts: [
        {
          draft: {
            id: "draft-tuesday",
            status: "ready",
            source: { sourceNoteId: SRC_CHAT, chatId: "chat-tuesday" },
            mutations: [],
          },
          freshness: "current",
          blockReasons: [],
      ...
      targets: [
        {
          noteId: "mem-tolley-refusal",
          title: "Tolley will not cross after the bell",
          noteType: "relationship",
          rows: [
            row(
              "draft-tuesday",
              write("m-tolley-1", "summary", "He said it twice on Tuesday and gave no reason either time."),
            ),
            row(
              "draft-tuesday",
              write("m-tolley-2", "continuity", "Mira did not argue with him, which is itself the point."),
            ),
            row(
              "draft-tuesday-late",
              write("m-tolley-held", "summary", "Held behind a stale draft; this row must never reach the queue."),
            ),
          ],
        },
        {
          ...
          rows: [
            row(
              "draft-tuesday",
              write("m-tuesday-1", "summary", "Nobody crossed. The market took the difference in unsold fish."),
            ),
            row(
              "draft-tuesday",
              mutation({
                id: "m-tuesday-link",
                kind: "add_link",
                summary: "Link to the crossing thread",
                link: { target: "mem-crossing-thread", relation: "concerns" },
              }),
            ),
          ],
        },
...
const imported = (
  over: Partial<Sample> & Pick<Sample, "sourceId" | "title" | "freshness">,
  existingNoteId: string,
): Sample => ({ ...sample(over), existingNoteId }) as Sample;
...
export const PREVIEWS: Record<string, ImportPreview> = {
  lorebooks: preview(
    ...
    [
      imported(
        {
          sourceId: "lb-atlas:e-harbour",
          title: "Lorebook - Atlas of the Harbour: The harbour",
          freshness: "current",
          snippet: "Sea-fog sits in the harbour until midday.",
          mutationCount: 3,
        },
        SRC_HARBOUR,
      ),
      imported(
        {
          sourceId: "lb-atlas:e-market",
          title: "Lorebook - Atlas of the Harbour: Fishmarket",
          freshness: "source_updated",
          snippet: "Open before light, shut by ten.",
          mutationCount: 1,
        },
        SRC_MARKET,
      ),
      sample({
        sourceId: "lb-atlas:e-fogline",
        title: "Lorebook - Atlas of the Harbour: The fog line",
        freshness: "new",
        snippet: "Past the fog line the lamps are useless.",
      }),
      sample({
        sourceId: "lb-atlas:e-tidebell",
        title: "Lorebook - Atlas of the Harbour: Tide bell",
        freshness: "new",
        snippet: "Rung twice for a returning boat.",
      }),
      sample({
        sourceId: "lb-cast:c-tolley",
        title: "Lorebook - Standing Cast: Tolley",
        freshness: "new",
        snippet: "Speaks in half sentences.",
      }),
      sample({
        sourceId: "lb-cast:c-chorus",
        title: "Lorebook - Standing Cast: Dock chorus",
        freshness: "new",
        snippet: "Six voices, no names, one opinion.",
      }),
    ],
    2,
  ),
  characters: preview(
    ...
    [
      ...
      sample({
        sourceId: "char-tolley",
        title: "Character - Tolley",
        freshness: "new",
        snippet: "Runs the ferry. Will not cross after the bell.",
      }),
      sample({
        sourceId: "char-merrow",
        title: "Character - Merrow",
        freshness: "new",
        snippet: "Keeps the far-shore light.",
      }),
    ],
    1,
  ),
  chats: preview(
    ...
    [
      imported(
        {
          sourceId: "chat-tuesday",
          title: "Tuesday crossing",
          freshness: "extraction_incomplete",
          importMode: "roleplay",
          snippet: "The ferry did not run.",
          mutationCount: 5,
        },
        SRC_CHAT,
      ),
      sample({
        sourceId: "chat-monday",
        title: "Monday, the long walk",
        freshness: "new",
        importMode: "roleplay",
        snippet: "Two hours around the head of the water.",
      }),
      sample({
        sourceId: "chat-market",
        title: "Market morning",
        freshness: "new",
        snippet: "Nothing happened, at length.",
      }),
    ],
    1,
  ),
