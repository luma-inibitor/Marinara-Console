...
const RECORDED: Record<string, string[]> = {
  ...
  "narrow/memory-review": ["1× button.notelink.t-data 18px"],
...
async function grade(page: Page): Promise<Report> {
  return page.evaluate(
    ([primary, secondary, gapFloor]) => {
      ...
      return {
        undersized,
        tightestGap: Number.isFinite(tightestGap) ? Math.round(tightestGap * 10) / 10 : null,
...
for (const screen of SCREENS) {
  test(screen.name, async ({ page }, info) => {
    ...
    expect(
      report.tightestGap,
      `${key} has no adjacent pair under ${TAP_GAP}px, so every small target grades secondary for free`,
