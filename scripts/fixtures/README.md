...
`deadexports` reads the whole real `src/` as consumers on every run, which a fixture cannot turn off. That is harmless — the real tree never imports from `scripts/fixtures`, and each fixture's own files are consumers of each other — but it does mean a fixture must export only what its case is about, or the extra symbols show up as findings of their own.
