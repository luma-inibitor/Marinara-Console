...
async function openPage(
  browser,
  /** @type {OpenOptions} */ { viewport, hash = "", url = DEV_URL + hash, settle = 0, timeout = 60000 },
) {
  const page = await browser.newPage({ viewport: { width: viewport.width, height: viewport.height } });
  let refused = new Set();
  page.on("requestfailed", (r) => {
    if (r.url().startsWith(DEV_URL)) refused.add(`${r.url()} ${r.failure()?.errorText ?? "failed"}`);
  });
  page.on("response", (r) => {
    if (r.status() >= 400 && r.url().startsWith(DEV_URL)) refused.add(`${r.url()} HTTP ${r.status()}`);
  });

  await page.goto(url, { waitUntil: "networkidle", timeout });
  for (let attempt = 0; ; attempt++) {
    try {
      await page.waitForFunction(() => (document.getElementById("app")?.childElementCount ?? 0) > 0, null, {
        timeout: 15000,
      });
      break;
    } catch {
  ...
  return page.evaluate(
    ([span, cap]) =>
      new Promise((resolve) => {
        ...
        const arm = () => {
          clearTimeout(idle);
          idle = setTimeout(() => stop(true), span);
        };
