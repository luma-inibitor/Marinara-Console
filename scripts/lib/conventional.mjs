// The Conventional Commits rule that the hook, the CI sweep and the title check share.

const TYPES = ["feat", "fix", "chore", "docs", "refactor", "style", "test", "perf", "build", "ci"];

// A capitalized type matches here and reports as a case error.
...
// The editor template's comment lines and the verbose diff are not the message.
...
export function conventionalProblem(subject) {
  ...
  const { type, scope, description } = /** @type {{ type: string, scope?: string, description: string }} */ (
    match.groups
  );
