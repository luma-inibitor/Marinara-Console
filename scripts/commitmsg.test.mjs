// These tests hold the commit subject rule in both directions.
...
describe("subjects that pass", () => {
  it.each([
    ...
    "style(ui): rewrite Chip as utilities",
  ])("accepts %s", (subject) => {
    expect(conventionalProblem(subject)).toBeNull();
  });

  it.each(["Merge pull request #12 from luma-inibitor/x", 'Revert "feat: add a thing"', "fixup! feat: add a thing"])(
    "skips git's own wording: %s",
    (subject) => {
...
describe("subjects that fail", () => {
  it.each([
    ...
    ["wip: park the half-done sheet", "`wip` is not a type used here"],
