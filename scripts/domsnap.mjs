...
const SURFACES = [
  ...
  { name: "sources-list", hash: "#/memory/sources", vps: [D], sel: ".srow", open: (p) => railAll(p) },
  {
    name: "sources-produced",
    hash: "#/memory/sources",
    vps: [D],
    sel: ".xbody",
    open: async (p) => {
      await railAll(p);
      await p.locator(".xchev").first().click();
    },
  },
  ...
  },
  {
    ...
    vps: [P],
    sel: '[role="menu"]',
    open: (p) => p.getByRole("button", { name: /^Group by: / }).click(),
  },
  ...
  { name: "palette", hash: "#/lorebooks", vps: [D], sel: ".palette", open: (p) => p.keyboard.press("Meta+k") },
  {
    name: "cheatsheet",
    hash: "#/lorebooks",
    vps: [D],
    sel: ".cheat",
    open: async (p) => {
      await p.locator(".rail-brand").click();
...
const UNREACHABLE = [
  ...
  ["obligations, failures (Review.tsx)", "blocked drafts and failed extractions. The dev queue has neither."],
  [
  ...
  ["toast row (Toaster.tsx)", "exists only while a toast is on screen, which races its own dismissal timer."],
  [
  ...
  ["group pressure (Review.tsx)", "a group whose target is near or over its cap. Nothing in the dev queue is."],
];
...
for (const key of Object.keys(base)) {
  ...
  const ca = count(a),
    cb = count(b),
    keys = new Set([...ca.keys(), ...cb.keys()]);
  const rows = [...keys]
    .filter((k) => (ca.get(k) ?? 0) !== (cb.get(k) ?? 0))
    .map((k) => `      ${ca.get(k) ?? 0} -> ${cb.get(k) ?? 0}  ${k}`);
  if (rows.length) {
    bad++;
    console.log(`  ${key}: ${a.length} -> ${b.length} elements`);
