#!/usr/bin/env node
// Checks a pull request title as a commit subject and its opening paragraph as prose.
...
export function bodyProblems(body) {
  ...
  const problems = [];
  // An abbreviation's period can only overcount the sentences.
  if ((opening.match(/[.!?](\s|$)/g) ?? []).length < 2)
    problems.push(
      "the opening paragraph is one sentence — it needs two: what the problem is, and what this does about it",
    );
  for (const [pattern, what] of JARGON)
    if (pattern.test(opening))
      problems.push(`the opening paragraph contains ${what} — it should read to someone who has not seen the code`);
  return problems;
}

// Only a direct run checks a pull request.
...
function main() {
  ...
  // GitHub sends a null body for an empty one.
